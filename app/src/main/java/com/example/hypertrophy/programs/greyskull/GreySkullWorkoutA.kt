...
fun GreySkullWorkoutALog(navController: NavHostController,
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(text = "GreySkull Workout A") }
            )
        },
        bottomBar = {
            Row(
        ...
        Column(
            ...
            var exercise3 = ExerciseData(exercisename = "Squat", setsAndReps = exercise3SetsAndReps)
            listOfExercises.add(exercise3)

            Spacer(modifier = Modifier.height(70.dp))

            var historyCard = HistoryCard(workoutTemplate = "GreySkull Workout A", date = todaysDate, exercises = listOfExercises)
